...
def list_sinks_sources():
    ...
    for line in lines.splitlines():
        if description_re.match(line):
            if "Display" in line.split("Description: ")[1]:
                display = False
            else:
                 last_device_match = line.split("Description: ")[1]
                 display=True

        elif mute_re.match(line) and display:
        ...
        elif volume_re.match(line) and display:
            volumes = line.split("/")
            ...
            if "Raptor" in last_device_match:
                last_device_match = "Laptop Speaker"
